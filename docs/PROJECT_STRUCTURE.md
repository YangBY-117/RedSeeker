...
- 架构形态：前端 Vue 应用通过 HTTP 调用后端 REST API；后端负责业务逻辑与算法。

-- 仓库内提供了用于本地集成测试的 SQLite 示例数据库与迁移脚本（见 `database/`），以 `red_tourism.db` 为单一本地测试 DB，并包含 `migrations/` 与用于生成/迁移用户数据的脚本。生产环境可替换为 MySQL/Postgres 等关系型数据库并使用相应的迁移工具（如 Flyway）。
